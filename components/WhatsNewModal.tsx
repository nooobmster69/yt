...
const parseMarkdownToHtml = (markdown: string): string => {
    ...
    for (const line of lines) {
        if (line.startsWith('# ')) {
            ...
            if (inList) { html += '</ul>'; inList = false; }
            html += `<p class="text-gray-600 dark:text-gray-400 my-2">${line}</p>`;
        }
    }
    if (inList) { html += '</ul>'; }
    return html;
    ...
    if (isOpen) {
      ...
      fetch('/CHANGELOG.md')
        ...
        .catch(err => {
            ...
            setContent('<p class="text-red-500">Could not load changelog. Please check the console for details.</p>');
            setIsLoading(false);
        });
    }
  }, [isOpen]);
      ...
      >
        <header className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 id="whats-new-modal-title" className="text-xl font-bold text-gray-900 dark:text-white">What's New in the Script Generator</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <main className="p-6 overflow-y-auto">
            {isLoading 
                ? <Loader message="Loading changelog..." /> 
